...
Page({
  ...
  onLoad: function (options) {
    ...
    that.setData({
      ...
      tel: wx.getStorageSync('userInfo').tel,
      img: wx.getStorageSync('userInfo').avatar,
  ...
  sendCode: function () {
    ...
    app.http('api/user/smscode', {
        token: wx.getStorageSync('openid'),
        phone_number: that.data.tel
      }, "POST")
      .then(res => {
  ...
  hiddenFloatView(e) {
    ...
    t = 0;
    this.setData({
      select: this.data.province + '-' + this.data.city + '-' + this.data.county
    })
    animationEvents(this, moveY, show);
